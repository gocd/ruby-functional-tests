...
step "Trigger stage <stage> run <run>" do |stage, run|
  begin
    response = RestClient.post http_url("/run/#{scenario_state.actual_pipeline_name(scenario_state.current_pipeline)}/#{run}/#{stage}"), "",basic_configuration.header
...
step "Cancel stage <stage> of pipeline <pipeline>" do |stage, pipeline|
  begin
    response = RestClient.post http_url("/api/stages/#{scenario_state.actual_pipeline_name(pipeline)}/#{stage}/cancel"),"", basic_configuration.header
    assert_true response.code == 200
  rescue RestClient::ExceptionWithResponse => err
