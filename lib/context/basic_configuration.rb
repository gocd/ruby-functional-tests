...
module Context
  class BasicConfiguration
    ...
    def set_material_path_for_pipeline(material_type, pipeline, material_path)
      ...
      current_config.xpath("//cruise/pipelines/pipeline[@name='#{scenario_state.retrieve(pipeline)}']/materials/#{material_type}").each do |material|
    ...
    def replace_pipeline_names
      config_dom.xpath('//cruise/pipelines/pipeline').each do |pipeline|
        ...
        scenario_state.store initial_name, pipeline['name']
      end
    end
    ...
    def remove_environments_except(except_environments)
      ...
      except_environments_names = except_environments.map do |except_env|
        scenario_state.retrieve(except_env)
      end
    ...
    def remove_pipelines_except(except_pipelines)
      ...
      except_pipelines_names = except_pipelines.map do |except_pipeline|
        scenario_state.retrieve(except_pipeline)
      end
    ...
    def header
      return {Confirm: true} unless scenario_state.retrieve 'current_user'
      basic_auth = Base64.encode64([scenario_state.retrieve('current_user'), 'badger'].join(':'))
      {Authorization: "Basic #{basic_auth}", Confirm: true}
    end
