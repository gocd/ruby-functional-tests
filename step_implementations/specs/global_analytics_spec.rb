...
step 'Verify pipeline <name> is shown on top of pipelines waiting longest for agents' do |pipeline_name|
	assert_true global_analytics_page.pipeline_with_highest_wait_time.casecmp(scenario_state.get(pipeline_name)).zero?
end
...
step 'Selecting pipeline <pipeline> should show pipeline build analytics' do |pipeline|
	global_analytics_page.pipeline_build_time_visible_on_selection scenario_state.get(pipeline)
end