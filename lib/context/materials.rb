...
module Context
  ...
  class ConfigRepoMaterial < Materials
    ...
    def setup(pipeline, repo, upstream)
      rm_rf(@path)
      mkdir_p(@path)
      actual_name = "#{pipeline}-#{SecureRandom.hex(4)}"
      cd(@path) do
      ...
      create_pipeline(actual_name, "#{@path}/config_repo.git", upstream)
      basic_configuration.set_config_repo("#{@path}/config_repo.git",repo)
      scenario_state.add_pipeline pipeline, actual_name
    end

    def create_pipeline(pipeline_name, material, upstream)
