...
step 'Verify SMTP server received email from <sender> to <recipient> with subject <subject>' do |sender, recipient, subject|
  message = smtp_server.wait_for_message(timeout: 5)
  ...
  assert_equal subject, message.subject, "Unexpected subject: #{message.subject}"
end
