...
module Context
  class Agents
    ...
    def run_agent_on_docker(identifier)
      ...
      unless !LOAD_BALANCED || ENV['RUN_ON_ALL_SERVER_IMAGE'] || manifest.has_image_count(GO_JOB_RUN_COUNT.to_i)
        raise "Expected job to be configured with GO_JOB_RUN_COUNT=#{manifest.image_count} (current=#{GO_JOB_RUN_COUNT}) as manifest implies that's how many agent images we have to test"
      end

      image_index = GO_JOB_RUN_INDEX ? GO_JOB_RUN_INDEX.to_i - 1 : 0
